...
import 'package:cricket_worlcup_ui/model/PCHistoryData.dart';
...
class _PCCricketHistoryScreenState extends State<PCCricketHistoryScreen> {
  ...
  init() async {
    String json = await rootBundle.loadString('assets/history.json');
                  ...
                  Expanded(
                    ...
                    child: Column(
                      children: <Widget>[
                        ...
                        RotatedBox(quarterTurns: -1, child: Text("${historyModel![index].champion} vs ${historyModel![index].runnersUp}", style: primaryTextStyle(color: Cricket_Primary, size: 12, fontFamily: fontRegular))),
                        Align(alignment: Alignment.center, child: Container(width: 1.0, height: 60, decoration: PCboxDecoration(bgColor: Colors.redAccent))),
                      ],
                    ),
                  ),
